...
function ResetPassword() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`${BASE_URL}/api/property/admin-reset-password/${id}/${token}`, { password: password });
      setLoading(false);
      
      if (res?.data && res?.data?.success === true) {
        cogoToast.success(`${res?.data?.message || 'Password reset successful!'}`);
        navigate("/admin_login");
        return;
      }
      if (res.data && res.data.success === false) {
        cogoToast.error(`${res?.data?.message || 'Password reset failed'}`);
        return;
      }
    } catch (err) {
      setLoading(false);
      ...
      <div className="login-container">
        <div className="login-card">
          <div className="card-header">
            ...
            <p>Enter your new password below to update credentials</p>
          </div>
          ...
          <div className="card-footer">
            <Link to="/admin_login" className="forgot-link">Return to login</Link>
          </div>
        </div>
      </div>
    </Wrapper>
